...
xlabel('x')
ylabel('y')
title(['Grid plots (Scale factor = ',num2str(Scale_factor),')'])
...
t_POD = [1,25,40,50,60,70,80,82,84,86,88,90,95,100,150];
N_POD = t_POD./dt;
k1 = 1;
...
% Find the column indices in X_cc(:,1) corresponding to x_range_POD
Nx_start = find(X_cc(1,:) >= x_range_POD(1), 1, 'first');
Nx_end   = find(X_cc(1,:) <= x_range_POD(2), 1, 'last');

% Find the row indices in Y_cc(:,1) corresponding to y_range_POD
Ny_start = find(Y_cc(:,1) >= y_range_POD(1), 1, 'first');
Ny_end   = find(Y_cc(:,1) <= y_range_POD(2), 1, 'last');


% Plot Velocity
t_plot = [1,10,20,40,70,100];
N_plot = t_plot./dt;
k2 = 1;
...
for n = 1:Nt
    ...
    RHS_p = Div_U_star./dt;
    tic
    [p_cc,Iter_count,RMS_residual] = PPE_solver_GS(p_cc,RHS_p,coeff,color,Nx,Ny,max_iter_PPE,tol_PPE);
    p_cc = PPE_solver_Matlab_solver(p_cc,RHS_p,A_Matrix,Corelation,color,Nx,Ny,max_iter_PPE,tol_PPE);
    ...
    if(n == N_POD(k1))
        u_POD{k1} = single(u_cc(Ny_start:Ny_end,Nx_start:Nx_end));
        v_POD{k1} = single(v_cc(Ny_start:Ny_end,Nx_start:Nx_end));
        k1 = k1 + 1;
    end
...
save(filename_POD,"u_POD","v_POD","X_cc","Y_cc","Ny_start","Ny_end","Nx_start","Nx_end","t_POD","dt");

%% Force plot
t = dt:dt:t_end;

% Force coefficient
C_D = 2.*Drag./(u_in^2*D_c);
C_L = 2.*Lift./(u_in^2*D_c);

filename_Force = "Force_coefficient_Re_" + num2str(Re) + ".mat";
